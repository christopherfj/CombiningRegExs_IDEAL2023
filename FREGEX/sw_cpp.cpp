...
class Combinations{
  public:
    ...
    void extract_tokens();
    void extend_tokens(string A, string B, vector<string> &tokens);
    // atributos
...
void Combinations::extend_tokens(string A, string B, vector<string> &tokens){
 ...
 void Combinations::extract_tokens(){
    ...
    //#pragma omp teams distribute parallel for
     for(index=0;index<posA.size();index++){
         ...
         */
         //#pragma omp critical
         extend_tokens(datosX[posA[index]], datosX[posB[index]], tokens);
     }
